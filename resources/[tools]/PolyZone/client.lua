...
function _drawWall(p1, p2, minZ, maxZ, r, g, b, a)
  ...
  Citizen.InvokeNative((`DRAW_POLY` & 0xFFFFFFFF), bottomLeft,topLeft,bottomRight,r,g,b,a)
  Citizen.InvokeNative((`DRAW_POLY` & 0xFFFFFFFF), topLeft,topRight,bottomRight,r,g,b,a)
  Citizen.InvokeNative((`DRAW_POLY` & 0xFFFFFFFF), bottomRight,topRight,topLeft,r,g,b,a)
  Citizen.InvokeNative((`DRAW_POLY` & 0xFFFFFFFF), bottomRight,topLeft,bottomLeft,r,g,b,a)
end
...
function PolyZone:draw()
  ...
  for i=1, #points do
    ...
    Citizen.InvokeNative((`DRAW_LINE` & 0xFFFFFFFF), point.x, point.y, minZ, point.x, point.y, maxZ, oR, oG, oB, 164)

    if i < #points then
      local p2 = self:TransformPoint(points[i+1])
  Citizen.InvokeNative((`DRAW_LINE` & 0xFFFFFFFF), point.x, point.y, maxZ, p2.x, p2.y, maxZ, oR, oG, oB, 184)
  ...
  if #points > 2 then
    ...
    Citizen.InvokeNative((`DRAW_LINE` & 0xFFFFFFFF), firstPoint.x, firstPoint.y, maxZ, lastPoint.x, lastPoint.y, maxZ, oR, oG, oB, 184)
...
local function _drawGrid(poly)
  ...
  for i=1, #lines do
    ...
    local min = line.min
    local max = line.max
    Citizen.InvokeNative((`DRAW_LINE` & 0xFFFFFFFF), min.x + 0.0, min.y + 0.0, maxZ + 0.0, max.x + 0.0, max.y + 0.0, maxZ + 0.0, r, g, b, 196)
  end
end
