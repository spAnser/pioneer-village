...
RegisterCommand(
  ...
  async (source: number, args: any[], rawCommand: string) => {
    ...
    const tick = setTick(async () => {
      ...
      // DisableControlAction(0, GetHashKey('INPUT_NEXT_CAMERA'), false);

      const leftRight = GetControlUnboundNormal(0, GetHashKey('INPUT_LOOK_LR')) * -1;
      heading += leftRight * 9;

      const upDown = GetControlUnboundNormal(0, GetHashKey('INPUT_LOOK_UD')) * -1;
      pitch += upDown * 4.5;
      pitch = Math.min(Math.max(pitch, -89), 89);

      SetEntityHeading(PlayerPedId(), heading);

      // PVCamera.setRot('first_person', { x: 0, y: 0, z: heading });
      PVCamera.setRot('first_person', { x: pitch, y: 0, z: heading });

      const forward = Vector3.fromArray(GetEntityForwardVector(PlayerPedId())).multiplyScalar(0.25);

      PVCamera.attachCamToPedBone('first_person', PlayerPedId(), 21030, forward, false);
    });

    PVCamera.create({
      id: 'first_person',
      coords: Vector3.fromArray(GetGameplayCamCoord()),
      rot: Vector3.fromArray(GetGameplayCamRot(2)),
      fov: GetGameplayCamFov(),
      _type: 'DEFAULT_SCRIPTED_CAMERA',
    });

    PVCamera.setActive('first_person');

    PVCamera.attachCamToPedBone('first_person', PlayerPedId(), 21030, { x: 0, y: -0.125, z: 0 }, false);
