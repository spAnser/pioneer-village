    ...
    CreateThread(function(threadId)
        ...
        CreateThread(function(threadId)
            ...
            while true do
                ...
                self.onRoad = Citizen.InvokeNative(0xE631EAF35828FA67, self.cache.playerId)

                if self.onRoad and not self.cache.onRoad then
                        self.cache.onRoad = true
                        TriggerEvent(("%s:onRoad"):format(self.name), true)
                elseif not self.onRoad and self.cache.onRoad then
                    self.cache.onRoad = false
