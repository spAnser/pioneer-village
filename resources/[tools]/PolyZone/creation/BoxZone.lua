local function handleInput(useZ, heading, length, width, center)
  if not useZ then
    ...
    if IsDisabledControlPressed(0, keys["CTRL"]) then -- ctrl held down
      scaleDelta, headingDelta = 0.05, 1
    end

    DisableControlAction(0, keys["SelectNextWeapon"], true)
    if IsDisabledControlJustPressed(0, keys["SelectNextWeapon"]) then
      if IsControlPressed(0, keys["LALT"]) then -- alt held down
        return heading, length, math.max(0.0, width - scaleDelta), center
      end
      if IsControlPressed(0, keys["SHIFT"]) then -- shift held down
        return heading, math.max(0.0, length - scaleDelta), width, center
      end
      return (heading - headingDelta) % 360, length, width, center
    end
    
    DisableControlAction(0, keys["SelectPrevWeapon"], true)
    if IsDisabledControlJustPressed(0, keys["SelectPrevWeapon"]) then
      if IsControlPressed(0, keys["LALT"]) then -- alt held down
        return heading, length, math.max(0.0, width + scaleDelta), center
      end
      if IsControlPressed(0, keys["SHIFT"]) then -- shift held down
...
function boxStart(name, heading, length, width, minHeight, maxHeight)
  ...
  Citizen.CreateThread(function()
    while createdZone do
      if IsControlJustPressed(0, keys["Z"]) then -- Z pressed
        useZ = not useZ
        if useZ then
